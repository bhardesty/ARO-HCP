...
func NewOrphanedBillingCleanupController(clock utilsclock.PassiveClock, billingClient database.BillingDBClient, clusterLister listers.ClusterLister, billingLister listers.BillingLister) controllerutils.Controller {
...
func (c *orphanedBillingCleanup) synchronizeAllBillingDocs(ctx context.Context) error {
	...
	for _, doc := range allBillingDocs {
		...
		err = c.billingClient.BillingDocs(doc.SubscriptionID).PatchByID(ctx, doc.ID, patchOperations)
		if err != nil {
			return utils.TrackError(err)
		}
