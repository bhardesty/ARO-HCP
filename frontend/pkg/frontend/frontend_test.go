...
func TestSubscriptionsGET(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			ts := newHTTPServer(ctx, f, mockARMResourcesDBClient, subs)
...
func TestRequestAdminCredential(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			_, err := mockARMResourcesDBClient.HCPClusters(clusterResourceID.SubscriptionID, clusterResourceID.ResourceGroupName).Create(ctx, cluster, nil)
			require.NoError(t, err)
			...
			if test.clusterProvisioningState.IsTerminal() && len(test.revokeCredentialsOperationID) > 0 {
				...
				_, err := mockARMResourcesDBClient.Operations(clusterResourceID.SubscriptionID).Create(ctx, revokeOp, nil)
				require.NoError(t, err)
			}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.clusterProvisioningState.IsTerminal() && len(test.revokeCredentialsOperationID) == 0 {
				...
				_, err := mockARMResourcesDBClient.Operations(clusterResourceID.SubscriptionID).Create(ctx, requestOp, nil)
				require.NoError(t, err)
			}
...
func newHTTPServer(ctx context.Context, f *Frontend, mockARMResourcesDBClient *databasetesting.MockARMResourcesDBClient, subs map[string]*arm.Subscription) *httptest.Server {
	...
	for _, sub := range subs {
		_, _ = mockARMResourcesDBClient.Subscriptions().Create(ctx, sub, nil)
	}
